...
export default function ArticlesPage() {
  ...
  // Création / modification directement dans une modal.
  // Aucun écran /new ou /[id]/edit n'est utilisé.
  ...
  async function openCreateModal() {
    ...
    try {
      ...
      const rows = (x: any) => {
        const d = x?.data ?? x;
        return Array.isArray(d) ? d : (d?.rows || d?.data || []);
      };
      setCategories(rows(c));
      setMarques(rows(m));
      setFournisseurs(rows(f));
    } catch {
      // Les listes restent facultatives : l'article peut toujours être créé.
    }
  }
  ...
  async function openEditModal(article: Article) {
    ...
    setArticleForm({
      ...
      oldPrice: article.old_price != null ? String(article.old_price) : "",
      purchasePrice: article.purchase_price != null ? String(article.purchase_price) : "",
      stock: article.stock != null ? String(article.stock) : "0",
      categoryId: article.category_id != null ? String(article.category_id) : "",
      marqueId: article.marque_id != null ? String(article.marque_id) : "",
      fournisseurId: article.fournisseur_id != null ? String(article.fournisseur_id) : "",
      status: String(article.status || "ACTIF").toUpperCase(),
  ...
  const filteredArticles = useMemo(() => {
    return articles.filter((article) => {
      ...
      if (selectedStock === "available" && stock <= 0) {
        return false;
      }
  ...
  const stats = useMemo(() => {
    const active = articles.filter(
      (article) =>
        normalizeStatus(article.status) === "active"
    ).length;
    ...
    const outOfStock = articles.filter(
      (article) => toNumber(article.stock) <= 0
    ).length;
 ...
 />
      ...
      {formOpen && (
        <div
          ...
          onMouseDown={(e) => {
            if (e.target === e.currentTarget && !savingArticle) setFormOpen(false);
          }}
        >
            ...
            <div className="p-6 sm:p-7">
              ...
              <div className="grid gap-5 lg:grid-cols-2">
                <Field label="Nom français *">
                  <input value={articleForm.name} onChange={e => setArticleForm({...articleForm,name:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10" placeholder="Ex. Crème hydratante" />
                </Field>
                <Field label="الاسم بالعربية" dir="rtl">
                  <input dir="rtl" value={articleForm.nameAr} onChange={e => setArticleForm({...articleForm,nameAr:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10" placeholder="اسم المنتج" />
                </Field>

                <Field label="Code article">
                  <input value={articleForm.code} onChange={e => setArticleForm({...articleForm,code:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10" placeholder="ART-001" />
                </Field>
                <Field label="SKU">
                  <input value={articleForm.sku} onChange={e => setArticleForm({...articleForm,sku:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10" placeholder="SKU-001" />
                </Field>

                <Field label="Prix de vente (DZD) *">
                  <input type="number" min="0" value={articleForm.price} onChange={e => setArticleForm({...articleForm,price:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10" placeholder="2500" />
                </Field>
                <Field label="Ancien prix (DZD)">
                  <input type="number" min="0" value={articleForm.oldPrice} onChange={e => setArticleForm({...articleForm,oldPrice:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10" placeholder="3000" />
                </Field>

                <Field label="Prix d'achat (DZD)">
                  <input type="number" min="0" value={articleForm.purchasePrice} onChange={e => setArticleForm({...articleForm,purchasePrice:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10" placeholder="1500" />
                </Field>
                <Field label="Stock">
                  <input type="number" min="0" value={articleForm.stock} onChange={e => setArticleForm({...articleForm,stock:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10" placeholder="0" />
                </Field>

                <Field label="Catégorie">
                  <select value={articleForm.categoryId} onChange={e => setArticleForm({...articleForm,categoryId:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10">
                    <option value="">Aucune catégorie</option>
                    {categories.map((x:any) => <option key={x.id} value={x.id}>{x.name}{x.name_ar ? ` / ${x.name_ar}` : ""}</option>)}
                  </select>
                </Field>
                <Field label="Marque">
                  <select value={articleForm.marqueId} onChange={e => setArticleForm({...articleForm,marqueId:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10">
                    <option value="">Aucune marque</option>
                    {marques.map((x:any) => <option key={x.id} value={x.id}>{x.name}{x.name_ar ? ` / ${x.name_ar}` : ""}</option>)}
                  </select>
                </Field>

                <Field label="Fournisseur">
                  <select value={articleForm.fournisseurId} onChange={e => setArticleForm({...articleForm,fournisseurId:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10">
                    <option value="">Aucun fournisseur</option>
                    {fournisseurs.map((x:any) => <option key={x.id} value={x.id}>{x.nom || x.name}</option>)}
                  </select>
                </Field>
                <Field label="Statut">
                  <select value={articleForm.status} onChange={e => setArticleForm({...articleForm,status:e.target.value})} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-semibold outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10">
              ...
              <label className="mt-5 flex cursor-pointer items-center gap-3 rounded-2xl bg-slate-50 p-4">
                ...
                />
                <span>
                  <b className="block text-xs font-black">Article mis en avant</b>
                  <small className="text-[10px] text-slate-400">Afficher cet article comme produit recommandé / à la une.</small>
                </span>
              </label>
              ...
              <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                >
                  {savingArticle ? <RefreshCw size={14} className="animate-spin" /> : <Save size={14} />}
                  {savingArticle ? "Enregistrement..." : editingArticle ? "Enregistrer les modifications" : "Créer l'article"}
                </button>
              </div>
            </div>
          </div>
