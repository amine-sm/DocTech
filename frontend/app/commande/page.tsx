...
export default function OrderPage() {
    ...
    async function loadDelivery() {
      ...
      try {
        ...
        for (const row of shipping.items ?? []) {
          ...
          const byId = String(row.wilayaId ?? "").trim();
          ...
          if (rowName) {
            const matchingWilaya = wilayaRows.find(
              (wilaya) =>
                String(wilaya.name ?? "").trim().toLowerCase() === rowName,
            );

            if (matchingWilaya) {
              costs[String(matchingWilaya.id)] = fee;
            }
          }
        }
  ...
  useEffect(() => {
    ...
    getDeliveryMunicipalities(selectedWilayaId)
      .then((rows) => {
        if (!cancelled) setDeliveryCommunes(rows);
      })
      .catch((error) => {
  ...
  const selectedShippingFee =
    selectedWilayaId && shippingCosts[selectedWilayaId] != null
      ? Number(shippingCosts[selectedWilayaId])
      : null;

  const deliveryFee =
    deliveryType === "home" && items.length && selectedShippingFee != null
      ? selectedShippingFee
      : 0;
  ...
  async function submitOrder(
    ...
    if (deliveryType === "home") {
      ...
      if (!wilayaId || !communeId || !address) {
        window.alert(
          text(
            "Veuillez renseigner la wilaya, la commune et l'adresse de livraison.",
            "يرجى إدخال الولاية والبلدية وعنوان التوصيل."
          )
        );
        return;
      }
      ...
      if (selectedShippingFee == null) {
        window.alert(
          text(
            "Les frais de livraison pour cette wilaya ne sont pas disponibles depuis Elogistia.",
            "سعر التوصيل لهذه الولاية غير متوفر حالياً من Elogistia."
          )
        );
        return;
      }
    ...
    <div className="min-h-screen bg-[#f3f6fb] text-slate-950">
      ...
      <main>
        <CheckoutHero
          ...
          description={text(
            "Vos produits sont réservés. Il ne reste plus qu’à indiquer vos coordonnées et choisir votre mode de réception.",
            "تم حجز منتجاتك. أدخل بياناتك واختر طريقة الاستلام."
          )}
              ...
              <div className="mt-3 flex items-end justify-between gap-4">
                <div>
                  ...
                  <span className="mt-1 block text-[9px] font-bold text-slate-400">
                    {text(
                      `Total TTC · ${totalQuantity} article${
                        totalQuantity > 1
                          ? "s"
                          : ""
                      }`,
                      `الإجمالي · ${totalQuantity} منتج`
                    )}
                  </span>
                </div>
        ...
        <section className="mx-auto max-w-[1450px] px-4 py-7 pb-28 sm:px-6 lg:px-8 lg:py-10 lg:pb-16">
          {!ready ? (
            ...
            <form
              ...
              className="grid items-start gap-7 lg:grid-cols-[minmax(0,1fr)_410px] xl:gap-10"
            >
              <div className="min-w-0 space-y-5">
                <CheckoutSection
                  ...
                  description={text(
                    "Ces informations servent uniquement à confirmer et livrer votre commande.",
                    "تُستخدم هذه المعلومات فقط لتأكيد طلبك وتوصيله."
                  )}
                ...
                <CheckoutSection
                  number="02"
                  title={text(
                    "Comment souhaitez-vous la recevoir ?",
                    "كيف تريد استلام الطلب؟"
                  )}
                  description={text(
                    "Choisissez l’option qui vous convient le mieux.",
                    "اختر طريقة الاستلام التي تناسبك."
                  )}
                  icon={<Truck size={19} />}
                >
                  <div className="grid gap-3 md:grid-cols-2">
                    ...
                    <DeliveryOption
                      ...
                      text={text(
                        "Récupérez votre commande à Es Sénia, Oran.",
                        "استلم طلبك من متجرنا في السانية، وهران."
                      )}
                      price={text(
                        "Gratuit",
                        "مجاني"
      ...
      <div className="overflow-hidden rounded-[30px] bg-[#07111f] text-white shadow-[0_28px_80px_rgba(15,23,42,0.18)]">
        ...
        <div className="border-t border-white/10 p-5 sm:p-6">
          ...
          <motion.button
            ...
            disabled={
              deliveryType === "home" &&
              (loadingDelivery || selectedShippingFee == null)
            }
            className={[
              "group mt-6 flex h-14 w-full items-center justify-center gap-2 rounded-[18px] px-5 text-[10px] font-black uppercase tracking-[0.08em] text-white shadow-lg transition",
              deliveryType === "home" &&
              (loadingDelivery || selectedShippingFee == null)
                ? "cursor-not-allowed bg-slate-400 shadow-none"
                : "bg-blue-500 shadow-blue-500/25 hover:bg-blue-400",
            ].join(" ")}
          >
            {loadingDelivery && deliveryType === "home"
              ? text("Chargement livraison…", "جاري تحميل التوصيل…")
...
function DeliveryOption({
  ...
  return (
    ...
    >
      ...
      <strong className="mt-4 block pe-16 text-[12px] font-black">
        {title}
      </strong>
  ...
  return (
    ...
    >
      ...
      <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-slate-500">
        Ajoutez quelques produits avant
        de passer à la finalisation.
      </p>
    ...
    <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_410px]">
      <div className="space-y-5">
        {[1, 2, 3].map((item) => (
          <div
            key={item}
            className="h-48 animate-pulse rounded-[28px] bg-white"
          />
        ))}
      </div>
