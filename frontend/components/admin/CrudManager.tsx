  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <button
          onClick={openCreate}
          className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 text-xs font-black text-white shadow-lg shadow-blue-600/20"
        >
          <Plus size={16} />
      ...
      <div className="overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row">
          ...
          <button
            onClick={load}
            className="flex h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 text-xs font-black text-slate-600"
          >
            <RefreshCw size={14} />
        ...
        {error && !modal && (
          <div className="m-4 rounded-xl bg-red-50 p-3 text-xs font-bold text-red-600">
            {error}
          </div>
        )}
                ...
                rows.map((row) => (
                  ...
                  >
                    {columns.map((column) => (
                      <td
                        key={column.key}
                        className="px-4 py-3 text-xs font-semibold text-slate-700"
                      >
                        {column.render
                          ? column.render(row)
                          : String(
              ...
              >
                <div className="space-y-3">
                  {columns.map((column) => (
                    ...
                    >
                      <span className="shrink-0 text-[9px] font-black uppercase tracking-[.11em] text-slate-400">
                        {column.label}
                      </span>
                    ...
                    {field.name ===
                      ...
                      <p className="mt-1.5 text-[10px] font-semibold text-slate-400">
                        {text(
                          ...
                          "يتم إنشاؤه تلقائيًا من الاسم الفرنسي."
                        )}
                      </p>
                    )}
                  </label>
