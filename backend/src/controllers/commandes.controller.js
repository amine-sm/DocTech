...
/* =========================================================
   STOCK — CONSOMMER (décrémenter les lots)
========================================================= */

async function consumeStockForOrder(conn, orderId, userId = null) {
  const [items] = await conn.query(
    "SELECT article_id, product_name, sku, quantity FROM commande_items WHERE commande_id=? ORDER BY id",
    [orderId]
  );
  ...
  for (const item of items) {
    ...
    const [[article]] = await conn.query(
      "SELECT id,name,purchase_price,price,stock_enabled FROM articles WHERE id=? FOR UPDATE",
      [item.article_id]
    );
    ...
    for (const lot of lots) {
      ...
      await conn.query(
        "UPDATE product_stock_lots SET quantity_remaining=quantity_remaining-? WHERE id=?",
        [take, lot.id]
      );

      await conn.query(
        `INSERT INTO stock_movements(
           article_id,lot_id,type,quantity,
           stock_before,stock_after,
           purchase_price,selling_price,
           supplier_id,reference,notes,user_id
         ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
        [
          item.article_id,
          lot.id,
    ...
    await conn.query(
      "UPDATE articles SET stock=GREATEST(0, stock-?) WHERE id=?",
      [qty, item.article_id]
    );
  }
}
...
/* =========================================================
   STOCK — RESTITUER (recréditer les lots)
========================================================= */

async function restoreStockForOrder(conn, orderId, userId = null) {
  const [items] = await conn.query(
    "SELECT article_id, product_name, sku, quantity, unit_price FROM commande_items WHERE commande_id=? ORDER BY id",
    [orderId]
  );
...
async function syncWithElogistia(orderId) {
  ...
  try {
    ...
    return {
      synced: false,
      error: error.message || "Erreur Elogistia",
    };
  }
}
...
/* =========================================================
   CREATE PUBLIC
   ⚠️ Le stock n'est PAS décrémenté ici.
   Il le sera quand la commande passera à CONFIRMEE.
========================================================= */
...
async function createPublic(req, res) {
  ...
  const deliveryType = ["HOME", "DESK", "STORE"].includes(
  ...
  if (deliveryType !== "STORE" && !b.wilayaId) {
    return res.status(400).json({
      ok: false,
      message: "La wilaya est obligatoire.",
    });
  }
  ...
  if (deliveryType === "HOME" || deliveryType === "DESK") {
    try {
      ...
      if (
        String(
          process.env.ELOGISTIA_REQUIRED_FOR_HOME || "true"
        ).toLowerCase() === "true"
      ) {
        ...
        for (const promo of promos) {
          const candidate =
            promo.type === "POURCENTAGE"
              ? Math.max(
                  0,
                  basePrice - (basePrice * Number(promo.value)) / 100
                )
  ...
  res.status(201).json({
    ...
    id: orderId,
    trackingNumber:
      created.delivery_tracking || created.tracking_number,
...
async function list(req, res) {
  ...
  const p = [];
  const w = status ? (p.push(status), "WHERE cmd.status=?") : "";

  const [[c]] = await pool.query(
    `SELECT COUNT(*) total FROM commandes cmd ${w}`,
    p
  );
...
/* =========================================================
   UPDATE STATUS — ⭐ GESTION AUTOMATIQUE DU STOCK
========================================================= */
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        oldStatus,
        newStatus,
        stockAction: !wasConsumed && willConsume
          ? "CONSUMED"
