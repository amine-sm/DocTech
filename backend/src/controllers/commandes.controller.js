...
function tracking(){return `DT-${new Date().getFullYear()}-${Math.random().toString(36).slice(2,8).toUpperCase()}`}
async function createPublic(req,res){const b=req.body;if(!b.customerName||!b.phone||!Array.isArray(b.items)||!b.items.length)return res.status(400).json({ok:false,message:'Client, téléphone et articles obligatoires.'});const conn=await pool.getConnection();try{await conn.beginTransaction();let subtotal=0;const prepared=[];for(const item of b.items){const [[a]]=await conn.query('SELECT id,name,sku,price,stock,stock_enabled,status FROM articles WHERE id=? FOR UPDATE',[item.articleId]);if(!a||a.status!=='ACTIF')throw Object.assign(new Error('Un article est indisponible.'),{status:400});const qty=Math.max(1,Number(item.quantity||1));if(a.stock_enabled&&a.stock<qty)throw Object.assign(new Error(`Stock insuffisant pour ${a.name}.`),{status:400});let price=a.price;if(item.variantId){const [[v]]=await conn.query('SELECT * FROM article_variants WHERE id=? AND article_id=? AND active=1',[item.variantId,a.id]);if(!v)throw Object.assign(new Error('Variante invalide.'),{status:400});if(v.price_override!=null)price=v.price_override}
    const [promos]=await conn.query(`SELECT pr.type,pr.value FROM promotions pr JOIN promotion_articles pa ON pa.promotion_id=pr.id WHERE pa.article_id=? AND pr.active=1 AND NOW() BETWEEN pr.start_at AND pr.end_at`,[a.id]);
    if(promos.length){
      const basePrice=Number(price);
      let bestPrice=basePrice;
      for(const promo of promos){
        const candidate=promo.type==='POURCENTAGE'?Math.max(0,basePrice-(basePrice*Number(promo.value)/100)):Math.max(0,basePrice-Number(promo.value));
...
async function track(req,res){const {trackingNumber,phone}=req.query;const [[c]]=await pool.query('SELECT tracking_number,status,customer_name,wilaya,commune,delivery_type,total,created_at,updated_at FROM commandes WHERE tracking_number=? AND phone=?',[trackingNumber,phone]);if(!c)return res.status(404).json({ok:false,message:'Commande introuvable.'});res.json({ok:true,data:c})}
module.exports={createPublic,list,getOne,updateStatus,track};
