...
function findShippingRow(rows, wilayaId, wilayaName) {
  const wantedId = String(wilayaId ?? "").trim().toLowerCase();
  const wantedName = String(wilayaName ?? "").trim().toLowerCase();

  return rows.find((item) => {
    const itemId = String(
      item?.wilayaID ?? item?.wilayaId ?? item?.wilaya_id ?? ""
    )
      .trim()
      .toLowerCase();

    const itemName = String(
      item?.wilayaLabel ?? item?.wilaya ?? item?.name ?? ""
    )
      .trim()
      .toLowerCase();
...
async function consumeStockForOrder(conn, orderId, userId = null) {
  ...
  for (const item of items) {
    ...
    if (!article || !article.stock_enabled) continue;
    ...
    const [lots] = await conn.query(
      `SELECT *
       ...
       WHERE article_id=? AND quantity_remaining>0
    ...
    for (const lot of lots) {
      ...
      await conn.query(
        `INSERT INTO stock_movements(
          article_id, lot_id, type, quantity, stock_before, stock_after,
          purchase_price, selling_price, supplier_id, reference, notes, user_id
        )
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
        [
          item.article_id,
    ...
    if (remaining > 0) {
      ...
      const [adjustLot] = await conn.query(
        `INSERT INTO product_stock_lots(
           ...
           purchase_price, selling_price, supplier_id, reference, notes
         )
         VALUES(?,?,?,?,?,?,?,?)`,
        [
          item.article_id,
...
async function restoreStockForOrder(conn, orderId, userId = null) {
  const [items] = await conn.query(
    `SELECT article_id, product_name, sku, quantity, unit_price
     FROM commande_items
     WHERE commande_id=?
     ORDER BY id`,
    [orderId]
  ...
  for (const item of items) {
    ...
    const [[lastExit]] = await conn.query(
      `SELECT lot_id
       ...
       WHERE article_id=? AND reference=? AND type='EXIT'
       ORDER BY id DESC
       LIMIT 1`,
      [item.article_id, `COMMANDE:${orderId}`]
    );
    ...
    if (lotId) {
      const [[existingLot]] = await conn.query(
        `SELECT id FROM product_stock_lots WHERE id=? FOR UPDATE`,
        [lotId]
      );
    ...
    await conn.query(
      ...
      [
        ...
        "ENTRY",
        qty,
        Number(stockRow?.stock || 0),
        Number(stockRow?.stock || 0) + qty,
        article.purchase_price || 0,
        article.price || 0,
        null,
        `COMMANDE:${orderId}`,
...
function splitCustomerName(customerName) {
  ...
  if (parts.length === 1) {
    return { name: parts[0], firstname: parts[0] };
  }
...
async function syncWithElogistia(orderId) {
  ...
  if (!order || order.delivery_type === "STORE") {
    return { synced: false, reason: "STORE" };
  }
  ...
  console.log("==============================================");
  console.log(`🚚 SYNCHRONISATION ELOGISTIA COMMANDE #${orderId}`);
  console.log("👤 CLIENT ELOGISTIA");
  console.log("   customer_name :", order.customer_name);
  console.log("   firstname     :", customer.firstname);
  console.log("   name          :", customer.name);
  console.log("   phone         :", order.phone);
  console.log("   wilaya        :", order.delivery_wilaya_id || order.wilaya);
  console.log("   commune       :", order.commune);
  console.log("==============================================");
  ...
  try {
    const result = await elogistia.createOrder({
      ...
      remarque: [
        order.note,
        order.delivery_agency_name ? `Bureau: ${order.delivery_agency_name}` : "",
      ]
        .filter(Boolean)
        .join(" | "),
      stop_desk:
      ...
      price: prices.join("|"),
      modeDeLivraison:
        order.delivery_mode || process.env.ELOGISTIA_DELIVERY_MODE || "4",
      exchangeName: "",
      idCommande: String(order.id),
      poids: process.env.ELOGISTIA_DEFAULT_WEIGHT || "1",
    });

    console.log("🎯 TRACKING ELOGISTIA:", result.tracking);
    ...
    console.error(`❌ ELOGISTIA COMMANDE #${orderId}`);
    ...
    return {
      synced: false,
      error:
        error.response?.data?.error || error.message || "Erreur Elogistia",
    };
  }
}
...
async function createPublic(req, res) {
  ...
  if (!b.customerName || !b.phone || !Array.isArray(b.items) || !b.items.length) {
    return res.status(400).json({
      ok: false,
      message: "Client, téléphone et articles obligatoires.",
    });
  }
  ...
  if (deliveryType === "HOME" || deliveryType === "DESK") {
    try {
      const shippingRaw = await elogistia.getShippingCost();
      const shippingRows = extractShippingRows(shippingRaw);
      const row = findShippingRow(shippingRows, b.wilayaId, b.wilaya);

      const selectedFee =
        deliveryType === "DESK"
          ? Number(row?.stopdesk ?? row?.stopDesk ?? row?.desk)
          : Number(row?.home);

      deliveryFee =
        Number.isFinite(selectedFee) && selectedFee >= 0
          ? selectedFee
          : Number(process.env.HOME_DELIVERY_FEE || 800);
    } catch (error) {
      console.error("ELOGISTIA SHIPPING ERROR:", error.message);

      if (
        String(process.env.ELOGISTIA_REQUIRED_FOR_HOME || "true").toLowerCase() ===
        "true"
      ) {
  ...
  try {
    ...
    for (const item of b.items) {
      const [[a]] = await conn.query(
        `SELECT id, name, sku, price, stock, stock_enabled, status
         FROM articles
         WHERE id=?
         FOR UPDATE`,
        [item.articleId]
      ...
      if (!a || a.status !== "ACTIF") {
        throw errorWithStatus("Un article est indisponible.");
      }
      ...
      if (item.variantId) {
        const [[v]] = await conn.query(
          `SELECT * FROM article_variants
           WHERE id=? AND article_id=? AND active=1`,
          [item.variantId, a.id]
        );
    ...
    const [r] = await conn.query(
      `INSERT INTO commandes(
        tracking_number, customer_name, phone, wilaya, commune, address, note,
        delivery_type, delivery_wilaya_id, delivery_commune_id, delivery_mode,
        delivery_stop_desk, delivery_agency_id, delivery_agency_name,
        delivery_provider, delivery_sync_status,
    ...
    for (const x of prepared) {
      await conn.query(
        `INSERT INTO commande_items(
          commande_id, article_id, variant_id, product_name, sku,
  ...
  if (deliveryType === "HOME" || deliveryType === "DESK") {
    provider = await syncWithElogistia(orderId);
  }

  const [[created]] = await pool.query(
    `SELECT
       id, tracking_number, delivery_tracking, status, delivery_type,
       wilaya, commune, address, subtotal, delivery_fee, total,
       delivery_sync_status, delivery_sync_error
     FROM commandes
     WHERE id=?`,
    [orderId]
  );
...
/* =========================================================
   LIST — avec filtres de dates + today
========================================================= */
...
async function list(req, res) {
  ...
  const today = String(req.query.today || "").toLowerCase() === "true";
  ...
  if (status) {
    conditions.push("cmd.status=?");
    p.push(status);
  }

  if (today) {
    conditions.push("DATE(cmd.created_at) = CURDATE()");
  } else {
    if (dateFrom) {
      conditions.push("DATE(cmd.created_at) >= ?");
      p.push(dateFrom);
    }
    if (dateTo) {
      conditions.push("DATE(cmd.created_at) <= ?");
  ...
  const [[c]] = await pool.query(
    `SELECT COUNT(*) total FROM commandes cmd ${where}`,
    p
  );
  ...
  const [rows] = await pool.query(
    `SELECT
       ...
       (
         ...
         LEFT JOIN article_images ai ON ai.article_id = ci.article_id
         WHERE ci.commande_id = cmd.id AND ai.url IS NOT NULL
  ...
  const enriched = rows.map((row) => ({
    ...row,
    items:
      row.first_item_image || row.first_item_name
        ? [
            {
              image: toAbsoluteUrl(row.first_item_image),
              product_name: row.first_item_name || "Article",
            },
          ]
        : [],
  }));
  ...
  res.json({
    ...
    pagination: {
      ...
      limit,
      total: c.total,
      pages: Math.ceil(c.total / limit),
    },
  });
}

/* =========================================================
   LIST MOVEMENTS — historique des changements de statut
========================================================= */
...
async function listMovements(req, res) {
  ...
  if (today) {
    conditions.push("DATE(h.created_at) = CURDATE()");
  } else {
    if (dateFrom) {
  ...
  if (orderId) {
    conditions.push("h.commande_id = ?");
    p.push(orderId);
  }

  if (status) {
    conditions.push("h.new_status = ?");
    p.push(status);
  }

  if (search) {
    conditions.push(
      "(cmd.tracking_number LIKE ? OR cmd.customer_name LIKE ? OR cmd.phone LIKE ?)"
    );
    const like = `%${search}%`;
  ...
  const [[c]] = await pool.query(
    `SELECT COUNT(*) total
     ...
     LEFT JOIN commandes cmd ON cmd.id = h.commande_id
     ${where}`,
    p
  );
...
async function getOne(req, res) {
  ...
  const [items] = await pool.query(
    `SELECT
       ci.id, ci.commande_id, ci.article_id, ci.variant_id,
       ci.product_name, ci.sku, ci.unit_price, ci.quantity, ci.line_total,

       (
         SELECT ai.url
         FROM article_images ai
         WHERE ai.article_id = ci.article_id
  ...
  const itemsWithImages = items.map((item) => {
    ...
    return { ...item, image_url: image, image };
  });

  // ✅ On renvoie aussi l'historique des statuts de cette commande
  const [history] = await pool.query(
    `SELECT id, old_status, new_status, user_id, user_name, created_at
  ...
  return res.json({
    ok: true,
    data: { ...c, items: itemsWithImages, history },
  });
}

/* =========================================================
   UPDATE STATUS — avec historique
========================================================= */
...
async function updateStatus(req, res) {
  ...
  try {
    ...
    const [[order]] = await conn.query(
      `SELECT id, status FROM commandes WHERE id=? FOR UPDATE`,
      [orderId]
    );
    ...
    if (!order) {
      ...
      return res
        .status(404)
        .json({ ok: false, message: "Commande introuvable." });
    }

    const oldStatus = String(order.status).toUpperCase();

    // ✅ Si le statut est identique → on ne crée pas de faux mouvement
    if (oldStatus === newStatus) {
      await conn.commit();
      return res.json({
        ok: true,
        message: "Statut inchangé.",
        data: { id: orderId, oldStatus, newStatus, stockAction: "NONE" },
      });
    }
    ...
    if (!wasConsumed && willConsume) {
      ...
      await consumeStockForOrder(conn, orderId, userId);
    }
    ...
    if (wasConsumed && !willConsume) {
      ...
      await restoreStockForOrder(conn, orderId, userId);
    }
    ...
    await conn.query(
      `INSERT INTO commande_status_history
        (commande_id, old_status, new_status, user_id, user_name)
       VALUES(?,?,?,?,?)`,
      [orderId, oldStatus, newStatus, userId, userName]
    );
...
async function track(req, res) {
  ...
  if (!trackingNumber || !phone) {
    return res.status(400).json({
      ok: false,
      message: "Numéro de suivi et téléphone obligatoires.",
    });
  }

  const [[c]] = await pool.query(
    `SELECT
       tracking_number, delivery_tracking, delivery_provider, status,
       customer_name, wilaya, commune, delivery_type, total,
  ...
  if (c.delivery_tracking) {
    try {
      ...
      providerTracking = remote?.data ?? remote?.body ?? remote;
    } catch (error) {
      console.error(
      ...
      providerTracking = {
        error: error.message || "Suivi Elogistia indisponible",
      };
    }
  }
